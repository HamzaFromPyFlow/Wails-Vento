...
function Landing() {
  return (
    <main className="min-h-screen bg-white text-slate-900 antialiased">
      ...
      <div className="fixed inset-0 bg-gradient-to-br from-slate-50 via-white to-indigo-50/40 pointer-events-none -z-10" />
      <div className="fixed inset-0 bg-[radial-gradient(ellipse_at_top,_var(--tw-gradient-stops))] from-indigo-200/20 via-transparent to-transparent pointer-events-none -z-10" />
      ...
      <section className="relative max-w-6xl mx-auto px-6 pt-20 pb-32">
        <div className="max-w-3xl">
          <p className="text-indigo-600 font-medium text-sm tracking-wide uppercase mb-4">
          ...
          <div className="flex flex-wrap gap-4">
            ...
            <button className="px-8 py-4 border border-slate-300 hover:border-slate-400 text-slate-700 hover:text-slate-900 font-medium rounded-xl transition-colors bg-white">
              Watch demo
            </button>
          </div>
        </div>
      ...
      <section id="features" className="max-w-6xl mx-auto px-6 py-24 scroll-mt-20">
        <h2 className="text-3xl font-bold mb-4 text-slate-900">Everything you need to stream</h2>
        ...
        <ul className="grid md:grid-cols-2 gap-8">
          {features.map((feature, i) => (
            <li
              key={i}
              className="p-6 rounded-xl border border-slate-200 bg-slate-50/80 hover:border-slate-300 transition-colors"
            >
      ...
      <section className="max-w-6xl mx-auto px-6 py-24">
        <div className="rounded-2xl border border-slate-200 bg-gradient-to-br from-indigo-50 to-purple-50 p-12 md:p-16 text-center">
          <h2 className="text-3xl font-bold mb-4 text-slate-900">Ready to go live?</h2>
      ...
      <section id="faq" className="max-w-6xl mx-auto px-6 py-24 scroll-mt-20">
        ...
        <p className="text-slate-600 mb-12">Quick answers to common questions.</p>
        <dl className="space-y-6">
          {faqs.map((faq, i) => (
            <div
              key={i}
              className="p-6 rounded-xl border border-slate-200 bg-slate-50/80"
            >
              <dt className="font-semibold text-lg mb-2 text-slate-900">{faq.q}</dt>
      ...
      <footer className="border-t border-slate-200 bg-slate-50/50">
        <div className="max-w-6xl mx-auto px-6 py-8 flex flex-col sm:flex-row items-center justify-between gap-4">
          <div className="flex items-center gap-2">
            ...
            <span className="font-medium text-slate-900">Vento</span>
          </div>
